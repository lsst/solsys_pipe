...
class ConsolidateTrackletsConnections(
    ...
    def dayobs_to_mjd(self, dayobs):
        dayobs = str(dayobs)
        assert len(dayobs) == 8, f'dayobs str "{dayobs}" not 8 chars'
        y = dayobs[:4]
        m = dayobs[4:6]
        d = dayobs[6:8]
    ...
    def adjust_all_quanta(self, adjuster):
        ...
        for data_id in to_do:
            ...
            if data_id['day_obs'] not in dayobs_to_dayobs_lists:
                # Skip quanta for day_obs which do not represent unique 
                # linking windows
                adjuster.remove_quantum(data_id)
                continue
            # In these loops, add all refs in this reference day_obs' dayobs_list
            # (i.e. in the past linkingWindow ) as inputs for this quantum.
        ...
        _LOG.info(
            f"Combined inputs from {len(day_obs)} day_obs into {len(dayobs_to_dayobs_lists)}"
            f"{self.config.linkingTimespan}-day windows: {dayobs_to_dayobs_lists}."
        )
